...
export const sendMailer = async (
  ...
  let info = await transporter.sendMail({
    ...
    html: `<h2>Xin chào ${data.email}</h2><br/>
            <h3>Cảm ơn bạn đã quan tâm đến dịch vụ khám và chăm sóc sức khỏe của Bookingcare</h3>
            <h2><b>Thông tin lịch hẹn của bạn:</b></h2><br/>
            <h3><b>Bác sĩ: ${doctorName}</b></h3><br/>
            <h3><b>Thời gian: ${timeType}</b></h3><br/>
            <h3>Xin vui lòng xác nhận trong đường link dưới đây để xác nhận lịch khám bệnh của bạn:</h3><br/>
            <a href="http://localhost:8000/booking/vetify?doctorId=${doctorId}&token=${token}" target="_blank">Click me!</a><br/>
            <h3>XIn chân thành cảm ơn</h3>`, // html body
  });
};
